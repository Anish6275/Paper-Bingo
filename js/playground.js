$(document).ready(function () {
        ...
        if (snapshot.val()) {
            ...
            if (Object.keys(snap).includes('checkedValue')) {
                Object.keys(snap.checkedValue).forEach(num => {
                    ...
                    boxes.forEach((b) => {
                        if ($(b).text() == num) {
                            $(b).attr('style', `background: ${snap.checkedValue[num].color};`);
                            maizeStatus[parseInt($(b).attr("data-box"))] = false;
                        }
                    });
                    check(maizeStatus);
                });
                $('#rec').html(`<span style="font-size: 1.5rem;">Recent:- </span><span class="badge secondary">${snap.recent}</span>`);
            }
            myColor = snap.users[name].color;
            clearInterval(timer);
            timer = setInterval(() => {
                per = Math.floor((time / 30000) * 100);
                $('#time').html(`<div class="bar warning w-${per}">${time / 1000}</div>`);
                time -= 1000;
                if (time < 0) {
                    clearInterval(timer);
                    next();
    ...
    $(document).on('click', '.box', function () {
        if (maizeStatus[parseInt($(this).attr("data-box"))] && name == currentUser && !winner.includes(name)) {
            ...
            checked = $(this).text();
            firebase.database().ref(`rooms/${localStorage.getItem('roomId')}/checkedValue/${checked}`).update({
                color: myColor,
                time: +new Date()
            }).then(() => {
    ...
    function check(cms) {
        ...
        if (bingo >= 5) {
            if (!winner.includes(name)) {
                winner.push(name);
                n = winner.length
                firebase.database().ref(`rooms/${localStorage.getItem('roomId')}/winner/${n}`).update({
                    name: name
                });
            }
        }
    ...
    function changeBox(deg, i) {
        $('*[data-box="' + i + '"]').attr('style', `background: linear-gradient(${deg}deg, #00f260, #0575e6);`);
    }
});
...
// linear-gradient(135deg, aquamarine, yellow)