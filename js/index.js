...
function checkRoomAndEnter() {
    if ($('#paperInputs2').val().trim() != '') {
        ...
        firebase.database().ref('rooms/' + $('#paperInputs2').val().trim().replace('-', '').toUpperCase()).once('value', function (snapshot) {
            if (snapshot.val()) {
...
function checkNameAndJoin() {
    if ($('#paperInputs3').val().trim() != '') {
        ...
        if (!Object.keys(snap).includes('users')) {
            ...
            window.location.replace('prepare.html');
        } else if (!Object.keys(snap.users).includes($('#paperInputs3').val().trim())) {
            localStorage.setItem('name', $('#paperInputs3').val().trim());
            window.location.replace('prepare.html');
        } else {
            pop('Sorry!!', '', 'There is someone with your name.. Please change it!!');
        }
        $('#btn_join').text('Join');
    } else {
        pop('Oaa!!', '', 'Enter Your Name!!');
